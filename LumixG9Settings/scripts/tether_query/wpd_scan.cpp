/**
 * wpd_scan — Comprehensive scan of Panasonic 0x9402 GetProperty codes
 *
 * Discovery from wpd_getprop: codes 0x020000XX work (groups 10,20,30,40,50).
 * This scanner tries all 0x020000XX groups systematically, plus other prefixes.
 * Also dumps 0x9108 ListProperty for each working group.
 *
 * Data format discovered:
 *   Each group returns multiple sub-properties packed as:
 *   [sub_id:4 LE][size_code:2 LE][pad:2][value:N bytes]
 *   where size_code: 02=2 bytes, 04=4 bytes, 08=8 bytes
 */
...
int main() {
    ...
    // Scan all 0x020000XX groups (XX = 00,10,20,...,F0)
    printf("\n=== Scanning 0x020000XX groups ===\n");
    for (UINT32 grp = 0x00; grp <= 0xFF; grp += 0x10) {
        UINT32 code = 0x02000000 | grp;
    ...
    // Also try individual codes 0x02000001..0x0200000F
    printf("\n=== Scanning 0x0200000X individual ===\n");
    for (UINT32 i = 0x01; i <= 0x0F; i++) {
    ...
    // Try other prefixes: 0x01, 0x03-0x0F, 0x10-0xFF
    printf("\n=== Scanning other prefixes 0xXX000010 ===\n");
    for (UINT32 prefix = 0x01; prefix <= 0xFF; prefix++) {
        if (prefix == 0x02) continue; // already scanned
        UINT32 code = (prefix << 24) | 0x10;
        ULONG resp = mtpRead(dev, 0x9402, buf, &sz, 1, code);
        if (resp == 0x2001 && sz > 0) {
            printf("  PREFIX 0x%02x: 0x%08x: %zuB OK\n", prefix, code, sz);
            decode_props(buf, sz);
        }
    }

    // Dump 0x9108 ListProperty for groups that had data
    printf("\n=== 0x9108 ListProperty for discovered groups ===\n");
    UINT32 groups[] = {0x02000010, 0x02000020, 0x02000030, 0x02000040, 0x02000050};
    for (UINT32 code : groups) {
        ULONG resp = mtpRead(dev, 0x9108, buf, &sz, 1, code);
        if (resp == 0x2001 && sz > 0) {
            printf("\n  LIST 0x%08x: %zuB\n", code, sz);
            hexdump(buf, sz < 4096 ? sz : 4096, 64);
        }
    }
